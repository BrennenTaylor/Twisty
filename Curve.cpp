...
Curve::Curve(uint32_t numSegments)
    : m_numSegments{numSegments}, m_arclength{0.0f},
      m_segmentLength(0.0f), m_basePos{0.0f, 0.0f, 0.0f}, m_baseTangent{0.0f,
                                                                        0.0f,
                                                                        0.0f},
      m_targetPos{0.0f, 0.0f, 0.0f}, m_targetTangent{0.0f, 0.0f, 0.0f},
      m_curvatures(m_numSegments), m_positions(m_numSegments + 1),
      m_tangents(m_numSegments + 1) {}

void Curve::CalculateFinalPosAndTangent(Farlor::Vector3 &finalPos,
                                        Farlor::Vector3 &finalDir) const {
  finalPos = m_targetPos;
  finalDir = m_targetTangent;
}

// Returns the polyline, as a vector of points, from the starting to end point
// with every segment point along the way The starting point and S_0's point are
// the same. Assumes that all the segments have up to date curvature, torsion
// and U matricies
void Curve::ReconstructCurvePositionsFirstOrder(
    std::vector<Farlor::Vector3> &positions) const {
  positions.clear();
...
void Curve::ReconstructCurvePositionsAndFramesFirstOrder(
    std::vector<Farlor::Vector3> &positions,
    std::vector<Farlor::Vector3> &tangents) const {
...
void Curve::WriteCurveToStream(std::ofstream &outputStream,
                               const twisty::Curve &seedCurve) {
  ...
  outputStream.write((char *)&seedCurve.m_curvatures[0],
                     sizeof(float) * seedCurve.m_numSegments);
  outputStream.write((char *)&seedCurve.m_positions[0],
                     sizeof(Farlor::Vector3) * (seedCurve.m_numSegments + 1));
  outputStream.write((char *)&seedCurve.m_tangents[0],
                     sizeof(Farlor::Vector3) * (seedCurve.m_numSegments + 1));
...
std::unique_ptr<Curve> Curve::LoadCurveFromStream(std::ifstream &inputStream) {
  ...
  inputStream.read((char *)&upInitialCurve->m_baseTangent,
                   sizeof(Farlor::Vector3));
  inputStream.read((char *)&upInitialCurve->m_targetPos,
                   sizeof(Farlor::Vector3));
  inputStream.read((char *)&upInitialCurve->m_targetTangent,
                   sizeof(Farlor::Vector3));

  inputStream.read((char *)&upInitialCurve->m_curvatures[0],
                   sizeof(float) * upInitialCurve->m_numSegments);
  inputStream.read((char *)&upInitialCurve->m_positions[0],
                   sizeof(Farlor::Vector3) *
                       (upInitialCurve->m_numSegments + 1));
  inputStream.read((char *)&upInitialCurve->m_tangents[0],
                   sizeof(Farlor::Vector3) *
                       (upInitialCurve->m_numSegments + 1));
...
twisty::PerturbUtils::BoundaryConditions_CudaSafe
Curve::GetBoundaryConditionsCudaSafe() const {
