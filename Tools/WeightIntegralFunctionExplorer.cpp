...
int main()
{
...

    {
        ...
        for (int idx = 0; idx < numStepsInt; idx++) {
            ...
            double result
                  = twisty::PathWeighting::IntegrandRT(currentP, kds_min, bds, eps, mu) * div;
            runningTotal += result;
            file << currentP << ',' << result << ',' << std::abs(result) << ','
                 << (runningTotal / numStepsInt) << '\n';
        }
        file.close();
    }

    {
        ...
        for (int idx = 0; idx < numStepsInt; idx++) {
            const float currentP = pStepHalf + (float)idx * pStep;
            double result
                  = twisty::PathWeighting::IntegrandRT(currentP, kds_max, bds, eps, mu) * div;
            runningTotal += result;
            file << currentP << ',' << result << ',' << std::abs(result) << ','
                 << (runningTotal / numStepsInt) << '\n';
        }
