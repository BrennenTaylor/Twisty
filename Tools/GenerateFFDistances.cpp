...
const float frameLength = 5.0;

int main(int argc, char *argv[])
{
    if (argc < 4)
    {
        fmt::print("Call as: {} imgWidth imgHeight outFilename", argv[0]);
        return 0;
    }
    ...
    for (uint32_t pixelIdxX = 0; pixelIdxX < imgWidth; ++pixelIdxX)
    {
        for (uint32_t pixelIdxY = 0; pixelIdxY < imgHeight; ++pixelIdxY)
        {
