...
int main(int argc, char *argv[]) {
  if (argc < 5) {
    std::cout << "Call as: " << argv[0]
              << " PathToExp numPathsInExperiment exportRate binaryFilename"
              << std::endl;
    return 1;
  }
  ...
  std::unique_ptr<twisty::Curve> upInitialCurve =
      std::make_unique<twisty::Curve>(numSegments);
  ...
  seedCurveFS.read((char *)&upInitialCurve->m_baseTangent,
                   sizeof(Farlor::Vector3));
  seedCurveFS.read((char *)&upInitialCurve->m_targetPos,
                   sizeof(Farlor::Vector3));
  seedCurveFS.read((char *)&upInitialCurve->m_targetTangent,
                   sizeof(Farlor::Vector3));

  seedCurveFS.read((char *)&upInitialCurve->m_curvatures[0],
  ...
  std::cout << "Initial Curve Base Pos: " << upInitialCurve->m_basePos
            << std::endl;
  std::cout << "Initial Curve End Pos: " << upInitialCurve->m_targetPos
            << std::endl;
  float segmentLength =
      upInitialCurve->m_arclength / upInitialCurve->m_numSegments;
  ...
  for (uint64_t i = 0; i < numPathsInExperiment; ++i) {
    ...
    if ((i % exportRate) == 0) {
      ...
      float segLength =
          upInitialCurve->m_arclength / upInitialCurve->m_numSegments;
      for (int64_t curvatureIdx = 0; curvatureIdx < numSegmentsPerPath;
           ++curvatureIdx) {
        Farlor::Vector3 tanLeft = tanBuffer[curvatureIdx];
        Farlor::Vector3 tanRight = tanBuffer[curvatureIdx + 1];

        Farlor::Vector3 curvatureVec =
            (tanRight - tanLeft) * (1.0f / segLength);
