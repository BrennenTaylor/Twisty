      ...
      uint32_t numSegments, float targetArclength) const
{
    ...
    upGeneratedCurve->m_ds = ds;

    upGeneratedCurve->m_boundaryConditions.m_startPos = GetStartPosition();
    upGeneratedCurve->m_boundaryConditions.m_startDir = GetStartNormal();
    upGeneratedCurve->m_boundaryConditions.m_endPos = GetTargetPosition();
    upGeneratedCurve->m_boundaryConditions.m_endDir = GetTargetNormal();
    upGeneratedCurve->m_boundaryConditions.arclength = arclength;
    ...
    {
        totalDistance += (upGeneratedCurve->m_boundaryConditions.m_endPos
              - upGeneratedCurve->m_positions[upGeneratedCurve->m_numSegments - 1])
                               .Magnitude();
    }
    std::cout << "\tSegment Curve Distance: " << totalDistance << std::endl;
    std::cout << "\tTarget Arclength: " << upGeneratedCurve->m_boundaryConditions.arclength
              << std::endl;
