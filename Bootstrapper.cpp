...
namespace twisty {
Bootstrapper::RayGeometry::RayGeometry(Farlor::Vector3 start,
                                       Farlor::Vector3 dir)
    : m_pos(start), m_dir(dir) {}

Bootstrapper::Geometry::SampleRay
Bootstrapper::RayGeometry::GetSampleRay() const {
  return SampleRay{m_pos, m_dir};
}

Bootstrapper::SphereGeometry::SphereGeometry(Farlor::Vector3 pos, float radius,
                                             float fov)
    : m_pos{pos}, m_radius{radius}, m_fov{fov} {}
...
Bootstrapper::Geometry::SampleRay
Bootstrapper::SphereGeometry::GetSampleRay() const {
...
Bootstrapper::Bootstrapper(
    const twisty::PerturbUtils::BoundaryConditions &problemGeometry)
...
Bootstrapper::CreateCurveGeometricSafe(uint32_t numSegments,
  ...
  // even
  const int remainingSegmentCount =
      evenNumberOfSegments ? numSegments - 2 : numSegments - 3;
  ...
  std::unique_ptr<Curve> upGeneratedCurve =
      std::make_unique<Curve>(numSegments);
  ...
  for (int leftIdx = 1; leftIdx <= remainingSegmentCountDiv2; ++leftIdx) {
    ...
    upGeneratedCurve->m_positions[leftIdx + xsPos] =
        x_s + leftIdx * ds * leftDir;
  }

  const Farlor::Vector3 rightDir = (x_em1 - x_t).Normalized();
  for (int rightIdx = 1; rightIdx <= remainingSegmentCountDiv2; ++rightIdx) {
    upGeneratedCurve
        ->m_positions[rightIdx + xsPos + remainingSegmentCountDiv2] =
  ...
  twisty::PerturbUtils::UpdateTangentsFromPos(
      upGeneratedCurve->m_positions.data(), upGeneratedCurve->m_tangents.data(),
      upGeneratedCurve->m_numSegments, boundaryConditions);

  double totalDistance = 0.0;
  // Calculate the actual length of the segment based curve
  for (uint32_t i = 0; i < upGeneratedCurve->m_numSegments - 1; ++i) {
    totalDistance += (upGeneratedCurve->m_positions[i + 1] -
                      upGeneratedCurve->m_positions[i])
                         .Magnitude();
  }
  // And handle last segment
  {
    totalDistance +=
        (upGeneratedCurve->m_targetPos -
         upGeneratedCurve->m_positions[upGeneratedCurve->m_numSegments - 1])
            .Magnitude();
  }
  std::cout << "\tSegment Curve Distance: " << totalDistance << std::endl;
  std::cout << "\tTarget Arclength: " << upGeneratedCurve->m_arclength
            << std::endl;
...
std::unique_ptr<Curve>
Bootstrapper::CreateCurve(uint32_t numSegments, float targetArclength,
  ...
  const Farlor::Vector3 x1x0 =
      m_experimentGeometry.m_endPos - m_experimentGeometry.m_startPos;
  ...
  const float shortestArclengthPossible =
      newBezierCurve.CalculateArclength(0.0f, 1.0f);

#if defined(DetailedCurveGen)
  std::cout << "\tShortest Arclength Before Moving cp2: "
            << shortestArclengthPossible << std::endl;
#endif
  ...
  targetArclength =
      std::max(static_cast<float>(targetArclength), shortestArclengthPossible) *
  ...
  newBezierCurve.m_controlPts[2] =
      newBezierCurve.m_controlPts[2] + n2 * guessVal;
  const float minBound = 0.0f;
  const float maxBound = 1.0f;
  const float finalArclength =
      newBezierCurve.CalculateArclength(minBound, maxBound);

#if defined(DetailedCurveGen)
  std::cout << "\tFinal arc length: " << finalArclength << std::endl;
  std::cout << "\tTarget arc length: " << targetArclength << std::endl;
  std::cout << "\tFinal bezier error: "
            << std::abs(finalArclength - targetArclength) << std::endl;
#endif
...
std::unique_ptr<Curve>
Bootstrapper::ToDiscreteFSCurve(uint32_t numSegments,
                                BezierCurve5 &bezierCurve) const {
  ...
  {
    upGeneratedCurve->m_positions[1] =
        m_experimentGeometry.m_startPos +
  ...
  for (uint32_t i = 2; i < numSegments; ++i) {
    ...
    float minF =
        bezierCurve.CalculateArclengthAlreadyCached(minT, a) - targetArclength;
    float maxF =
        bezierCurve.CalculateArclengthAlreadyCached(minT, b) - targetArclength;
    ...
    float guessVal = (a + b) / 2.0f;
    while (currentIterationCount < maxNumberOfIterations) {
      float aVal = bezierCurve.CalculateArclengthAlreadyCached(minT, a) -
                   targetArclength;
      float bVal = bezierCurve.CalculateArclengthAlreadyCached(minT, b) -
                   targetArclength;
      float guessF =
          bezierCurve.CalculateArclengthAlreadyCached(minT, guessVal) -
          targetArclength;
  ...
  for (uint32_t i = 0; i < upGeneratedCurve->m_numSegments; ++i) {
    Farlor::Vector3 diff = (upGeneratedCurve->m_positions[i + 1] -
                            upGeneratedCurve->m_positions[i]);
  ...
  {
    upGeneratedCurve->m_tangents[numSegments] =
        m_experimentGeometry.m_endDir.Normalized();
  }
