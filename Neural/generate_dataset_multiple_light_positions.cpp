...
void DoSingleLight(const int lightIdx, const bool generateImages, std::ofstream &outfile,
      std::filesystem::path datasetDir, const int numDataSetPairs, const int numDirectionsPerSample,
      std::mt19937 &generator)
{
    std::uniform_real_distribution uniform01(0.0f, 1.0f);
    datasetDir /= std::to_string(lightIdx);
    ...
    std::uniform_real_distribution<float> lightDist(-40.0f, 40.0f);
    Farlor::Vector3 lightPos { lightDist(generator), lightDist(generator), lightDist(generator) };

    for (int dataPairIdx = 0; dataPairIdx < numDataSetPairs; dataPairIdx++) {
        if ((dataPairIdx % 10) == 0) {
            std::cout << "\r Light Idx " << lightIdx << " Dataset: "
    ...
    if (generateImages) {
        ...
        std::ostringstream imageSamplesFilenameSS;
        imageSamplesFilenameSS << datasetDir.string();
        imageSamplesFilenameSS << "/image_samples_" << lightIdx << ".csv";
        const std::string imageSamplesFilename = imageSamplesFilenameSS.str();
        ...
        std::ostringstream gtFilenameSS;
        gtFilenameSS << datasetDir.string();
        gtFilenameSS << "/gt_" << lightIdx << ".exr";
        const std::string gtFilename = gtFilenameSS.str();
...
int main(int argc, char **argv)
{
    ...
    const uint32_t numLightSamples = std::stoi(argv[3]);
    const bool generateImages = std::stoi(argv[4]);

    std::string datasetDirName("Multiple_Light_Dataset");
    if (argc > 5) {
        datasetDirName = std::string(argv[5]);
    }

    std::mt19937 generator { seed };

    std::filesystem::path currentDir = std::filesystem::current_path();
    currentDir /= datasetDirName;
    ...
    for (int lightIdx = 0; lightIdx < numLightSamples; lightIdx++) {
        DoSingleLight(lightIdx, generateImages, outfile, currentDir, numDataSetPairs,
              numDirectionsPerSample, generator);
        std::cout << std::endl;
    }
