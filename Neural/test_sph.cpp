...
constexpr uint32_t numSamplesSqrt = 5000;
...
float P(int l, int m, float x)
{
    ...
    float ppmp1 = x * (2.0f * m + 1.0f) * ppm;
    if (l == m + 1)
        return ppmp1;

    float pll = 0.0f;
    for (int ll = m + 2; ll <= l; ++ll) {
        pll = ((2.0f * ll - 1.0f) * x * ppmp1 - (ll + m - 1.0f) * ppm) / (ll - m);
        ppm = ppmp1;
        ppmp1 = pll;
    }
    return pll;
...
float K(int l, int m)
{
    float temp = ((2.0f * l + 1.0f) * std::tgammaf(l - m + 1))
          / (4.0f * std::numbers::pi_v<float> * std::tgammaf(l + m + 1));
...
float SH(int l, int m, float theta, float phi)
{
    ...
    if (m == 0) {
        ...
        return sqrt2 * K(l, m) * cos(m * phi) * P(l, m, std::cos(theta));
    } else {
...
int main()
{
    ...
    for (uint32_t sampleIdx = 0; sampleIdx < numSamplesSqrt * numSamplesSqrt; sampleIdx++) {
        ...
        // std::cout << "Sample coefficients: " << std::endl;
        // for (auto &coefficient : sample.coeff) {
