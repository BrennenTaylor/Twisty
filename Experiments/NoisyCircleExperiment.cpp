...
int main(int argc, char *argv[])
{
    if (argc < 13) {
        std::cout << "Call as: " << argv[0]
                  << " numPathsPerInternalExperiment numPathsToSkipPerInternalExperiment "
                     "experimentName bootstrapperSeed perturbSeed normalGenSeed arclengthGenSeed "
                     "startX startY startArclength frameLength framePixelCount numInitialCurves "
                     "numPerInitialCurve"
                  << std::endl;
        return 1;
    }
                ...
                for (uint32_t arclengthIdx = 0; arclengthIdx < numArclengths; ++arclengthIdx) {
                    ...
                    for (uint32_t initialCurveIdx = 0; initialCurveIdx < numInitialCurves;
                        ...
                        for (uint32_t perInitialCurveIdx = 0;
                              perInitialCurveIdx < numPerInitialCurve; ++perInitialCurveIdx) {
