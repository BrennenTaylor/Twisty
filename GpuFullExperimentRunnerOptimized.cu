...
namespace twisty
{
    ...
    __global__ void GpuFullExperimentRunnerOptimized::InitializeCurandState(uint32_t seed, curandState_t* pStates, uint32_t maxNumStates)
    {
        // Unique index
    ...
    __global__ void GpuFullExperimentRunnerOptimized::GeneratePathBatchPutrubations(
...
namespace twisty
{
    ...
    bool GpuFullExperimentRunnerOptimized::SetupCurveDataStructures()
    {
        ...
        for (uint32_t segmentIdx = 0; segmentIdx < m_experimentParams.numSegmentsPerCurve; ++segmentIdx)
        {
            ...
            initialCurvePositionsAndTangents[segmentIdx]._pad = 0.0f;
            ...
            initialCurvePositionsAndTangents[segmentIdx + m_experimentParams.numSegmentsPerCurve + 1]._pad = 0.0f;
        ...
        initialCurvePositionsAndTangents[m_experimentParams.numSegmentsPerCurve]._pad = 0.0f;
        ...
        initialCurvePositionsAndTangents[m_experimentParams.numSegmentsPerCurve * 2 + 1]._pad = 0.0f;
    ...
    void GpuFullExperimentRunnerOptimized::WeightCombineThreadKernel(const uint32_t threadIdx, uint32_t numWeights, uint32_t numWeightsPerThread, const std::vector<float>& compressedWeights, twisty::BigFloat& threadWeight)
    {
    ...
    ExperimentRunner::ExperimentResults GpuFullExperimentRunnerOptimized::RunExperiment()
    {
        ...
        std::cout << "We have " << numHardwareThreads << " avalible for weight combining." << std::endl;

        uint32_t numWeightsPerThread = (compressedWeightBuffer.size() + numHardwareThreads - 1) / numHardwareThreads;
        ...
        for (uint32_t threadIdx = 0; threadIdx < numHardwareThreads; ++threadIdx)
        {
            ...
            std::thread newThread(&GpuFullExperimentRunnerOptimized::WeightCombineThreadKernel, this, threadIdx, compressedWeightBuffer.size(), numWeightsPerThread, std::ref(compressedWeightBuffer), std::ref(threadWeights[threadIdx]));
