...
BaseWeightLookupTable::BaseWeightLookupTable(
    const WeightingParameters &weightingParams, double ds, double minCurvature,
    double maxCurvature)
    : m_minCurvature(minCurvature), m_maxCurvature(maxCurvature),
      m_curvatureStepSize(0.0),
      m_lookupTable(weightingParams.numCurvatureSteps +
                    1) // We include 0, thus need that last spot
      ,
      m_ds(ds), m_weightingParams(weightingParams) {
  m_curvatureStepSize = (m_maxCurvature - m_minCurvature) /
                        (weightingParams.numCurvatureSteps - 1);
...
void BaseWeightLookupTable::ExportValues(std::string directoryFileName) {
  ...
  const std::filesystem::path outputFilePath =
      exprDirectory / tableValuesFilename;
...
WeightLookupTableIntegral::WeightLookupTableIntegral(
  ...
  std::cout << "\t\tm_numCurvatureSteps: " << weightingParams.numCurvatureSteps
            << std::endl;
}
...
double
WeightLookupTableIntegral::Integrate(double curvature,
                                     const WeightingParameters &weightingParams,
                                     double ds) const {
  double kds = curvature * ds;
  double bds = weightingParams.scatter * ds;

  auto Integrand = [this, weightingParams](double p, double kds,
                                           double bds) -> double {
    double phaseFunction = GaussianPhase(p, weightingParams.mu);

    double scatteringTerm =
        p * std::exp(bds * phaseFunction // scatter piece
                     - 1.0 * (weightingParams.eps * weightingParams.eps * p *
                              p * 0.5) // regularizer
            );
...
SimpleWeightLookupTable::SimpleWeightLookupTable(
  ...
  auto CalculateSimpleWeightValue = [weightingParams,
                                     ds](double curvature) -> double {
    curvature *= -1.0; // All curvatures in this mode are negated
    const double alpha =
        1.0 / (weightingParams.scatter * ds * weightingParams.mu);
...
MinMaxCurvature CalcMinMaxCurvature(const twisty::WeightingParameters &wp,
                                    double ds) {
...
NormalizerDoubleType psd_one(const BaseNormalizer &fn, int M, const double s,
                             const Farlor::Vector3 &X, const Farlor::Vector3 &N,
                             const Farlor::Vector3 &Np,
...
NormalizerDoubleType Norm(const BaseNormalizer &fn, int M, double z, double s) {
  ...
  NormalizerDoubleType piPower = boost::multiprecision::pow(
      boost::multiprecision::cpp_dec_float_100(2.0) *
...
NormalizerDoubleType likelihood(const BaseNormalizer &fn, int M,
                                const double arclength,
                                const Farlor::Vector3 &endPosition,
                                const Farlor::Vector3 &startPosition,
                                const Farlor::Vector3 &N,
                                const Farlor::Vector3 &Np,
                                const std::vector<Farlor::Vector3> &oldBetas,
                                const std::vector<Farlor::Vector3> &newBetas) {
  Farlor::Vector3 Z0 =
      (endPosition - startPosition) * (M + 1) * (1.0 / arclength);
...
NormalizerDoubleType CalculateLikelihood(
    const BaseNormalizer &fn, int numSegments,
    const twisty::PerturbUtils::BoundaryConditions &boundaryConditions,
    std::vector<Farlor::Vector3> &oldBetas,
    std::vector<Farlor::Vector3> &newBetas) {
...
std::unique_ptr<PathWeighting::NormalizerStuff::BaseNormalizer>
GetNormalizer(uint32_t numSegments) {
  const std::string fnFilename = "SavedFN.fnd";
  const std::filesystem::path fnFilePath =
      std::filesystem::current_path() / fnFilename;
