...
struct WeightingParameters {
  ...
  WeightingParameters() : scatterValues() { scatterValues.push_back(0.0); }
};
...
public:
  WeightLookupTableIntegral(const WeightingParameters &weightingParams,
                            double ds);
  virtual ~WeightLookupTableIntegral();

protected:
  double Integrate(double curvature, const WeightingParameters &weightingParams,
                   double ds) const;
...
public:
  SimpleWeightLookupTable(const WeightingParameters &weightingParams,
                          double ds);
...
public:
  FN(int samples, int numIntegrationSamples, int orders, const double &rmn,
     const double &rmx)
      : nb_samples(samples), nb_orders(orders), rmin(rmn), rmax(rmx) {
    init(numIntegrationSamples);
  }

  FN(std::ifstream &inFile) : nb_samples(0), nb_orders(0), rmin(0), rmax(0) {
...
NormalizerDoubleType
likelihood(const BaseNormalizer &fn, int numSegments, const double arclength,
...
NormalizerDoubleType CalculateLikelihood(
    ...
    std::vector<Farlor::Vector3> &oldTangents,
    std::vector<Farlor::Vector3> &newTangents);
