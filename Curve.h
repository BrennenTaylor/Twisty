...
public:
  ...
  /**
   * @brief Itegrates over the curve using a first order method, obtaining
   * position and frame values for each segment.
   *
   ...
   */
  void ReconstructCurvePositionsAndFramesFirstOrder(
   ...
   * @param frames Out-parameter of frames of each integrated segment
   */
  void ReconstructCurvePositionsFirstOrder(
      std::vector<Farlor::Vector3> &positions) const;

  void CalculateFinalPosAndTangent(Farlor::Vector3 &finalPos,
  ...
  static void WriteCurveToStream(std::ofstream &outputStream,
  ...
  GetBoundaryConditionsCudaSafe() const;
...
public:
  ...
  /*
      ...
      M + 1 position values
          ...
          The second position is always the start position + start direction *
     ds The final position is always the target position M + 1 direction values
